...
Interr supports both web and React Native platforms with platform-specific file extensions and directories:
...
### System Requirements
- **Node.js and npm** are required (Node >= 22, npm >= 10)
- Development server runs at http://localhost:8080/

### Development Workflow
- Development server proxies API/signalling to configurable target (default: https://alpha.jitsi.net)
...
### Tests
- End-to-end tests are defined in the `tests/` directory
- Tests cover peer-to-peer and multi-participant (JVB) scenarios on web and mobile platforms
...
- Extensive configuration options available in `config.js`
- `interface_config.js` for UI/branding options
